...
class GstoreConnector:
    ...
    def get(self, params, stream=False):
        if stream:
            return GstoreConnector.error_handling(requests.get(self.base_url, params=params, stream=stream))
        else:
            return GstoreConnector.error_handling(requests.get(self.base_url, params=parse.urlencode(params, quote_via=parse.quote), stream=stream).text)

    def post(self, params, stream=False):
        if stream:
            return GstoreConnector.error_handling(requests.post(self.base_url, json=params, stream=stream))
        else:
            return GstoreConnector.error_handling(requests.post(self.base_url, json=params, stream=stream).text)
